...
template<class T>
std::ostream& operator << (std::ostream& stream, const chrono::ChVector<T>& tmpvec)
{
    stream << tmpvec.x << " ";
    stream << tmpvec.y << " ";
    stream << tmpvec.z;
...
template<class T>
std::ostream& operator << (std::ostream& stream, const chrono::ChQuaternion<T>& tmpq)
{
    stream << tmpq.e0 << " ";
    stream << tmpq.e1 << " ";
    stream << tmpq.e2 << " "; 
...
inline double length(const chrono::ChVector<>& v1)
{
    return sqrt(v1.Dot(v1));
}
