...
public:
  ...
  double Get_y(double curr_t) {
    double desired_angle = GetPatternAngle(curr_t);
    double desired_angular_speed = GetPatternAngularSpeed(curr_t);
...
protected:
  double GetPatternAngle(double t) {
    const double phase =
        double(engine_->GetIdentifier() * 2) / body_list_.size() * CH_C_2PI;
