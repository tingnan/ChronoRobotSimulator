...
public:
    ...
    bool BroadCallback(
        collision::ChCollisionModel*  mmodelA,	///< pass 1st model
...
int main(int argc, char *argv[])
{
    ...
    ChIrrApp application(&my_system, L"A simple RFT example", core::dimension2d<u32>(1024, 768), false, true, video::EDT_OPENGL);
    ChIrrWizard::add_typical_Logo(application.GetDevice());
    ChIrrWizard::add_typical_Sky(application.GetDevice());
    ChIrrWizard::add_typical_Lights(application.GetDevice());
    const double sp = atof(argv[3]);
    ChIrrWizard::add_typical_Camera(application.GetDevice(), core::vector3df(5 * sp, 10 * sp, 0), core::vector3df(5 * sp, 0, 0));
    ...
    while (application.GetDevice()->run() && my_system.GetChTime() < 30)
    {
        ...
        if (count == savestep)
        {
            ...
            std::cout << std::fixed << std::setprecision(4) << my_system.GetChTime() << std::endl;
