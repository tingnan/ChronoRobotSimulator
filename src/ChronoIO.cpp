...
void ChronoIOManager::DumpJntInfo()
{
    ...
    for (itr = mChSys->Get_linklist()->begin(); itr != mChSys->Get_linklist()->end(); ++itr)
    {
...
void ChronoIOManager::DumpContact()
{
    ...
    ChContactContainer* syscot = (ChContactContainer*)(mChSys->GetContactContainer());
    ...
    {
        collision::ChModelBulletBody* modelA = (collision::ChModelBulletBody*)((*itr)->GetModelA());
        collision::ChModelBulletBody* modelB = (collision::ChModelBulletBody*)((*itr)->GetModelB());
        ChBody* bodyA = modelA->GetBody();
        ChBody* bodyB = modelB->GetBody();
        ChVector<> curforce = 
            (*itr)->GetContactCoords().TransformDirectionLocalToParent((*itr)->GetContactForce());
