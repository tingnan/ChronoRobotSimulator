...
// transform the rft segments (generated from the mesher) from its body frame to local frame
void RFTSegsTransform(RFTBody &rbody,
    std::vector<ChVector<> >& pos_list,
    ...
    std::vector<ChVector<> >& for_list)
{
    ...
    if (true)
    {
        ...
        for (int i = 0; i < npiece; ++i)
        {
            ...
            ori_list[i] = chbody->TransformPointLocalToParent(rbody.olist_[i]) - chbody->GetPos();
            nor_list[i] = ori_list[i];
            for_list[i] = chbody->TransformPointLocalToParent(ChVector<>(1, 0, 0)) - chbody->GetPos();
        }
        //debugfile << std::flush;
    }
...
void DrawVector(irr::ChIrrApp *mApp, const ChVector<>& pos, const ChVector<>& foc, double scale, int color)
{
    ...
    driver->draw3DLine(irr::core::vector3dfCH(pos), irr::core::vector3dfCH(pos + foc * scale), mcol);
}
...
void RFTSystem::InteractExt(RFTBody& rbody)
{
    ...
    double poffset = 0;
    ChBody& chbody = *rbody.GetChBody();
    // std::cout << chbody.GetIdentifier() << " " << chbody.GetCollide() << std::endl;
    if (RFTTestCollision(chbody, ydir_, poffset) && chbody.GetCollide())
    {
        ...
        for (int i = 0; i < nn; ++i)
        {
            InteractPiece(pos_list[i], vel_list[i], ori_list[i], nor_list[i], for_list[i], rbody.alist_[i], poffset, rbody.flist_[i]);
...
void ForceHu(double deltah, double cospsi, double sinpsi, double area, double *fnorm, double *fpara)
{
    ...
    *fpara = prefac * (mu_f * hevistep(sinpsi) + mu_b * (1 - hevistep(sinpsi))) * sinpsi;
}
...
void ForceBB(double deltah, double cospsi, double sinpsi, double area, double *fnorm, double *fpara)
{
    ...
    double CF = 1.34;
    double CL = -0.82;
    *fnorm = prefac * CS * gamma * cospsi / sqrt(1 + gamma * gamma * cospsi * cospsi);
    *fpara = 1 * prefac * (CF * sinpsi + CL * (1 - cospsi));
}

void RFTSystem::InteractPiece(
    const ChVector<> &pos,
    const ChVector<> &vel,
    const ChVector<> &ori,
    const ChVector<> &nor,
