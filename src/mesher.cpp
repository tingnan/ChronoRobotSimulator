...
void MeshBox(const btBoxShape *cshape, 
    double envelope, std::vector<ChVector<> > &plist, std::vector<ChVector<> > &olist, std::vector<double> &alist)
{
...
void SetRFTMesh(chrono::ChBody *pbody, std::vector<ChVector<> > &plist, std::vector<ChVector<> > &olist, std::vector<double> &alist)
{
    ...
    if (strcmp(cbody->GetBulletModel()->getCollisionShape()->getName(), "Box") == 0)
    {
