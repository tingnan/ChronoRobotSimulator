...
MyEventReceiver::MyEventReceiver(ChIrrApp* myapp, GlobalControlSet *myset)
{
    ...
    mygui->addStaticText(L"pause physics:", 
    	core::rect<s32>(220, 10, 300, 30), false, false, 0, 101)->setTextAlignment(EGUIA_CENTER, EGUIA_CENTER);
    mChBoxPauseSim = mygui->addCheckBox(false, core::rect<s32>(300, 10, 320, 30), 0, 102, L"");
    mApp->SetPaused(mChBoxPauseSim->isChecked());    

    mygui->addStaticText(L"time step:", 
    	core::rect<s32>(330, 10, 380, 30), false, false, 0, 103)->setTextAlignment(EGUIA_CENTER, EGUIA_CENTER);
    mEdBoxTimeStep = mygui->addEditBox(L"1e-3", core::rect<s32>(380, 10, 430, 30), true, 0, 104);
    mEdBoxTimeStep->setTextAlignment(EGUIA_CENTER, EGUIA_CENTER);
    mControlSet->gTimeStep = wcstod(mEdBoxTimeStep->getText(), NULL);

    mygui->addStaticText(L"k:", 
    	core::rect<s32>(440, 10, 470, 30), false, false, 0, 105)->setTextAlignment(EGUIA_CENTER, EGUIA_CENTER);
    mEdBox_k = mygui->addEditBox(L"25", core::rect<s32>(470, 10, 500, 30), true, 0, 106);
    mEdBox_k->setTextAlignment(EGUIA_CENTER, EGUIA_CENTER);
    mControlSet->snakeParams.k = wcstod(mEdBox_k->getText(), NULL);

    mygui->addStaticText(L"A:", 
    	core::rect<s32>(510, 10, 540, 30), false, false, 0, 107)->setTextAlignment(EGUIA_CENTER, EGUIA_CENTER);
    mEdBox_A = mygui->addEditBox(L"0.50", core::rect<s32>(540, 10, 570, 30), true, 0, 108);
    mEdBox_A->setTextAlignment(EGUIA_CENTER, EGUIA_CENTER);
    mControlSet->snakeParams.A = wcstod(mEdBox_A->getText(), NULL);

    mygui->addStaticText(L"freq:", 
    	core::rect<s32>(580, 10, 620, 30), false, false, 0, 109)->setTextAlignment(EGUIA_CENTER, EGUIA_CENTER);
    mEdBox_w = mygui->addEditBox(L"0.2", core::rect<s32>(620, 10, 650, 30), true, 0, 110);
    mEdBox_w->setTextAlignment(EGUIA_CENTER, EGUIA_CENTER);
    mControlSet->snakeParams.w = wcstod(mEdBox_w->getText(), NULL);
    
    mygui->addStaticText(L"height:",
        core::rect<s32>(660, 10, 700, 30), false, false, 0, 111)->setTextAlignment(EGUIA_CENTER, EGUIA_CENTER);
    mEdBox_h = mygui->addEditBox(L"-0.26", core::rect<s32>(700, 10, 740, 30), true, 0, 112);
