...
void ForceHu(double deltah, double cospsi, double sinpsi, double area,
  ...
  double prefac = 9.81 * deltah * area * 7e2;
...
RFTSystem::RFTSystem(irr::ChIrrApp *ch_app)
  ...
  // ChVector<> force;
  // ChVector<> position(0, -1, 0);
  // ChVector<> velocity(1, 0, 0);
  // for (size_t i = 0; i < 100; ++i) {
  //   double theta = double(i) / 100.0 * chrono::CH_C_PI_2;
  //   ChVector<> normal(cos(theta), 0, sin(theta));
  //   InteractPieceHorizontal(position, velocity, normal, true, 1., force);
  //   debug_file << theta << " " << force << std::endl;
  // }
  // exit(0);
}
